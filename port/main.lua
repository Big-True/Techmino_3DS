-- port/main.lua — Techmino 3DS port
-- This file REPLACES Techmino/main.lua in the 3DS build.
-- compat.lua was already loaded by conf.lua (runs first).
--
-- Changes from upstream Techmino/main.lua:
--   1. jit.* references guarded (line 175)
--   2. love.setDeprecationOutput guarded (line 32)
--   3. love.keyboard.setKeyRepeat guarded (line 33)
--   4. Shader loading wrapped in pcall (lines 91-97)
--   5. Discord RPC guarded (lines 614-615)
--   6. Virtual keys enabled by default on 3DS
--   7. mouse.isDown in cursor rendering guarded (line 169)
--
-- All other patches (missing APIs, stubs) are in compat.lua
...
pcall(function() love.setDeprecationOutput(false) end)
pcall(function() love.keyboard.setKeyRepeat(true) end)
...
-- 3DS: shaders are stubbed (newShader returns dummy objects via compat.lua)
SHADER={}
for _,v in next,fs.getDirectoryItems('parts/shaders') do
    if FILE.isSafe('parts/shaders/'..v) then
        local name=v:sub(1,-6)
        if love.graphics.newShader then
            local ok,shader=pcall(love.graphics.newShader,'parts/shaders/'..name..'.glsl')
            if ok then SHADER[name]=shader end
        end
    end
end
...
do -- Z.setCursor
    ...
    Z.setCursor(function(time,x,y)
        if not SETTING.sysCursor then
            ...
            GC.setColor(1,1,1)
            -- 3DS: love.mouse.isDown replaced with touch check
            local isHolding = love.mouse.isDown and love.mouse.isDown(1) or false
            GC.draw(isHolding and holdImg or normImg,x,y,nil,nil,nil,8,8)
        end
    end)
end
Z.setOnFnKeys({
    ...
    function()
        if jit then
            ...
            MES.new('info',("System:%s\nluaVer:%s"):format(SYSTEM,_VERSION))
        end
    end,
...
-- 3DS: Enable virtual keys by default (touch screen is the primary input)
if HANDHELD then
    SETTING.VKSwitch=true
    SETTING.powerInfo=true
end
...
-- Discord RPC (stubbed on 3DS via compat.lua preload)
