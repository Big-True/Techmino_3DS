...
do-- function applySettings()
    ...
    function applySettings(reason)
        ...
        if SHADER.blockSatur then SHADER.blockSatur:send('b',m[1]); SHADER.blockSatur:send('k',m[2]) end
        m=saturateValues[SETTING.fieldSatur] or saturateValues.normal
        if SHADER.fieldSatur then SHADER.fieldSatur:send('b',m[1]); SHADER.fieldSatur:send('k',m[2]) end
